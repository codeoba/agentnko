  ...
  sock.ev.on('messages.upsert', async (m) => {
    ...
    for (const msg of m.messages) {
      try {
        ...
        console.log(`AI Config check for user ${userId}: enabled=${aiConfig?.enabled}, contact.ai_disabled=${contact?.ai_disabled}, ruleBypassedAI=${ruleBypassedAI}`);
        ...
        if (aiConfig && aiConfig.enabled === 1 && contact.ai_disabled !== 1 && !ruleBypassedAI) {
          ...
          try {
            ...
            console.log(`Plan check for user ${userId}: plan=${user?.plan}, active_until=${user?.active_until}, isPlanActive=${isPlanActive}`);
...
- Pia ikiwa utathibitisha kuorder kwa mteja (kutoa namba ya order), hakikisha unaongeza tag hii mwishoni mwa ujumbe ili mfumo wetu utengeneze PDF stakabadhi kiotomatiki: [RECEIPT_DATA: {"items":[{"name":"Sukari","price":3000,"quantity":2}],"discount":0,"customerName":"Jina la Mteja","deliveryAddress":"Anuani"}] - data iwe na muundo huu wa JSON sahihi.`;
            ...
            console.log(`AI execution failed for user ${userId}:`, aiErr);
          }
        }
      } catch (fatalErr) {
        console.log(`FATAL ERROR processing message for user ${userId}:`, fatalErr);
      }
    }
  });
